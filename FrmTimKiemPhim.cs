...
namespace QuanLyRapPhim
{
    public partial class FrmTimKiemPhim : Form
    {
        ...
        private void Load_comboBox()
        {
            ...
            cboTheLoai.DataSource = DAO.GetDataToTable("SELECT MaTheLoai, TenTheLoai FROM tblTheLoai");
            cboTheLoai.ValueMember = "MaTheLoai";
            cboTheLoai.DisplayMember = "TenTheLoai";
            cboTheLoai.SelectedIndex = -1;

            //Load cboMaHangSX tìm kiếm
            cboHangSX.DataSource = DAO.GetDataToTable("SELECT MaHangSX, TenHangSX FROM tblHangSX");
        ...
        private void btnTimkiem_Click(object sender, EventArgs e)
        {
            if (txtTenphim.Text == "")
            {
                MessageBox.Show("Hãy chọn tên Phim cần tìm!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (cboTheLoai.Text == "")
            {
                MessageBox.Show("Hãy chọn tên thể loại cần tìm!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            if (cboHangSX.Text == "")
            {
                MessageBox.Show("Hãy chọn tên hãng sản xuất cần tìm!", "Thông Báo", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return;
            }
            else
            { 
